...
Pattern::Pattern(char *filename) {
    int v1, v2, max, min;
    std::vector<int> int_vector;
    std::ifstream infile(filename);
    infile >> max >> min;
    int_vector.push_back(max);
    int_vector.push_back(min);
    while (infile >> v1 >> v2) {
        int_vector.push_back(v1);
        int_vector.push_back(v2);
        if (v1 > v2) {
            max = (v1 > max) ? v1 : max;
            min = (v2 < min) ? v2 : min;
        } else {
    ...
    adj_mat = new int[size * size];
    memset(adj_mat, 0, size * size * sizeof(int));
    for(size_t i = 0; i < int_vector.size(); i+=2) {
        add_edge(int_vector[i] - min, int_vector[i+1] - min);
        add_edge(int_vector[i+1] - min, int_vector[i] - min);
    }
}
